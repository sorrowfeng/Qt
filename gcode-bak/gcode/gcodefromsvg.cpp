...
{
    // 这里elementsByTagName有问题, 还没找到替代
//   foreach (QDomElement pathElement, svgCode->documentElement().elementsByTagName("path"))
...
//           cout << id;
           ...
           if (d.length() > 0)
           {
               ...
               if (svgPauseElement || svgPausePenDown) { /*gcode.Pause(gcodeString, "Pause before path");*/ }
               QRegExp separators("(?=[A-Za-z-[e]])");  // 正则有问题, 作用是将各个命令分隔开来
...
//        cout << "transform里的内容: " << tstring;
        ...
        {
            QRegExp re("(?<=[\\)])");   // 正则有问题, 匹配不出正确的空格
//            QRegExp re("(?<=\\))[ ](?=.)");

            QStringList tlist = tstring.split(re);  // 这里匹配的是第一个空格前的字符串, 这里匹配失败了
//            cout << tlist << tlist.length() << tlist.first();
...
//    QPoint points[outputSegmentCount + 1];
    QPoint * points = new QPoint;
    for (int i = 0; i <= outputSegmentCount; i++)
    {
        double t = (double)i / outputSegmentCount;
        points[i] = GetBezierPoint(t, controlPoints, 0, controlPoints->manhattanLength());
    }
    return points;  //  有点问题
}
...
{
//    svgPath = "M 8.7185884,-4.0017078";     // 测试代码
...
//    cout << remainingargs;
//    QString argSeparators = @"[\s,]|(?=(?<!e)-)";// @"[\s,]|(?=-)|(-{,2})";        // support also -1.2e-3 orig. @"[\s,]|(?=-)";

//    var splitArgs = Regex
//        .Split(remainingargs, argSeparators)
//        .Where(t => !string.IsNullOrEmpty(t));
    QRegExp re("[\\s,]|(?=(?<!e)-)");       // 正则有问题
    QStringList splitArgs = remainingargs.split(re);
    splitArgs.removeAll("");

//    QStringList splitArgs;
//    splitArgs << "8.7185884" << "-4.0017078";   // 测试用
//    cout << splitArgs;
...
//        cout << floatArgs[i];
//        cout << sizeof (floatArgs)/sizeof (floatArgs[0]);
    }
    int objCount = 0;
